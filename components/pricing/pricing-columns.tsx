    ...
    <div className={cn("grid items-stretch gap-6 md:gap-8 lg:grid-cols-3", className)}>
      ...
      <Card className="h-full flex flex-col overflow-visible">
        <CardHeader>
          ...
          <CardTitle className="mt-1 text-lg md:text-xl">Report Packages</CardTitle>
        ...
        <CardContent className="flex-1 overflow-visible">
          <div className="space-y-3">
            ...
            <div className="rounded-xl border border-border bg-muted/50 p-4">
              <div className="h-[40px] flex items-end gap-1 whitespace-nowrap">
                <span className="text-4xl font-bold leading-none">${currentPackage.price}</span>
                <span className="text-sm text-muted-foreground leading-none pb-[2px]">${currentPackage.pricePerReport?.toFixed(2)}/report</span>
              </div>
      ...
      <Card className="relative h-full flex flex-col border-primary shadow-lg">
        ...
        <CardHeader>
          <div className="flex items-center gap-2">
            <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Limited Time</p>
            <span className="text-xs font-medium text-destructive animate-pulse">
              • Ends in <PromoTimer />
            </span>
          </div>
          <CardTitle className="mt-1 text-lg md:text-xl">{proPass.name}</CardTitle>
        ...
        <CardContent className="flex-1">
          <div className="space-y-3">
            <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
              <div>
                <span className="text-5xl font-semibold">${proPass.price}</span>

              </div>
              <p className="mt-2 text-sm font-medium text-primary">Generate reports for 72 hours</p>
            </div>
          </div>
      ...
      <Card className="relative h-full flex flex-col border-primary/50">
        ...
        <CardHeader>
          <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Pro Access</p>
          <CardTitle className="mt-1 text-lg md:text-xl">{monthlyPlan.name}</CardTitle>
        ...
        <CardContent className="flex-1">
          <div className="space-y-3">
            <div className="rounded-xl border border-border bg-muted/50 p-4">
              <div>
                <span className="text-5xl font-semibold">${monthlyPlan.price}</span>
...
function PromoTimer() {
  // Initialize with a random duration between 4 and 12 hours in seconds to simulate "ending soon"
  // kept consistent via state to avoid hydration mismatch, ideally fixed timestamp
  const [seconds, setSeconds] = useState(16452)

  useEffect(() => {
    const timer = setInterval(() => {
      setSeconds((prev) => (prev > 0 ? prev - 1 : 16452))
    }, 1000)
